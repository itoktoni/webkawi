    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div class="col-md-12 col-lg-12">
                ...
                <div class="form-group">
                    <label class="col-md-2 control-label">Penjahit</label>
                   <div class="col-md-4 {{ $errors->has('production_id') ? 'has-error' : ''}}">
                    <select class="form-control col-md-4 option" name="production">
                        @if(Auth::user()->group_user != 'production')
                        <option value="">Select Penjahit</option>
                        @endif
                        @foreach($production as $value)
                        <option @isset($data) {{ $value->production_id == $data->supplier_name ? 'selected="selected"' : '' }} @endisset value="{{ $value->production_id }}">{{ $value->production_name }}</option>
                        @endforeach
                    </select>
                    </div>
